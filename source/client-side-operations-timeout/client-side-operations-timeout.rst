...
MUST also document that overriding ``timeoutMS`` for operations executed
using the explicit session inside the provided callback will result in a
...
overridden.  This was a conscious API design decision because drivers are
...
via an option similar to ``timeoutMode`` for cursors. To avoid
